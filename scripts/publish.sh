...
REMOTE_VERSION=$(npm show @fusionworks/react-admin-google-maps version)
...
if [[ $PACKAGE_VERSION != $REMOTE_VERSION ]]; then
  ...
  npm run release
  # https://github.com/FusionWorks/react-admin-google-maps
  git push "https://${GITHUB_TOKEN}@github.com/FusionWorks/react-admin-google-maps.git" master
